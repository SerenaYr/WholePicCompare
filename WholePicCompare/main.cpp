	...
	vector<Rect> textTingJianGengDuo, vector<Rect> picBottonIcon)
{
	...
	const Rect rectPicSearch(Point(0, 50), Point(width, 130));		//1.搜索框
	const Rect rectTextTuiJian(Point(0, 155), Point(width, 230));	//2.“乐库推荐趴间看点”文字
	const Rect rectPicTuiJian(Point(0, 270), Point(width, 500));	//3.推荐图片
	const Rect rectPicFourFunc(Point(0, 520), Point(width, 645));	//4.今日30首等4种功能图片
	const Rect rectTextTuiJianGeDan(Point(0, 700), Point(width, 790));	//5.“推荐歌单”文字
	const Rect rectPicTextTuiJianGeDan(Point(0, 800), Point(width, 1520));	//6.推荐图片
	...
	const Rect rectTextMV(Point(0, 3400), Point(width, 3530));				//10.“MV”文字
	const Rect rectPicMV(Point(0, 3540), Point(width, 4600));				//11.“MV”图片
	const Rect rectTextKanDian(Point(0, 4650), Point(width, 4770));			//12.“看点”文字
	const Rect rectPicKanDian(Point(0, 4780), Point(width, 5900));			//13.“看点”图片
	const Rect rectTextYinYueRen(Point(0, 5970), Point(width, 6070));		//14.“看点”图片
	const Rect rectPicYinYueRen(Point(0, 6070), Point(width, 7200));		//15.“看点”图片
	const Rect rectTextPaJian(Point(0, 7300), Point(width, 7400));			//16.“趴间”图片
	const Rect rectPicPaJian(Point(0, 7400), Point(width, 8500));			//17.趴间图片
	...
	for (auto countourPicSearch : countoursPicSearch)
	{
		...
		cv::rectangle(mImg, tmpRectPicSearch, Scalar(255, 0, 0), 8);
	}
	...
	cv::rectangle(mImg, picTuiJian, Scalar(255, 0, 0), 8);
	...
	for (auto countourFourFunc : countoursFourFunc)
	{
		...
		cv::rectangle(mImg, tmpPicFourFunc, Scalar(255, 0, 0), 8);
	}
...
void main()
{
	...
	Mat srcImg = imread("MyPic.jpg");	//读入客户端图像
	...
	// 对客户端图片进行位置检测标注
	DealModel(srcImg2, picSearch, textTuiJian, picTuiJian, picFourFunc, textTuiJianGeDan, picTuiJianGeDan,
		textXinTuiJian, picXinTuiJianLeft, picXinTuiJianRight, textMV, picMV, textKanDian, picKanDian,
		textYinYueRen, picYinYueRen, textPaJian, picPaJian, textTingJianGengDuo, picBottonIcon);
	...
	imwrite("标记边框视觉图.jpg", objImg, compression_params);
	imwrite("标记边框客户端图.jpg", srcImg2, compression_params);
